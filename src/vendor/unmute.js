...
/**
 * Enables unmute.
 * @param {AudioContext} context A reference to the web audio context to "unmute".
 * @param {boolean} [allowBackgroundPlayback] Optional. Default false. Allows audio to continue to play in the background. This is not recommended because it will burn battery and display playback controls on the iOS lockscreen.
 * @param {boolean} [forceIOSBehavior] Optional. Default false. Forces behavior to that which is on iOS. This *could* be used in the event the iOS detection fails (which it shouldn't). It is strongly recommended NOT to use this.
