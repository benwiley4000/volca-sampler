...
int convertSample(char *filename, uint8_t *inputArray, uint32_t bytes, uint32_t slotNumber, bool useCompression)
{
  SampleBufferContainer *sampleBuffer = prepareSampleBufferFromWavData(inputArray, bytes, slotNumber, 16, useCompression ? 1 : 0);
  if (!sampleBuffer)
  {
    printf("Oops\n");
    return 1;
...
int main(int argc, char **argv)
{
  ...
  int afterLastSlashIndex = lastSlashIndex(wavfilename, strlen(wavfilename)) + 1;
